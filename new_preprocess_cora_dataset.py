...
def parse_cora_content(in_file, train_percentage):
  ...
  with open(in_file, 'rU') as cora_content:
    for line in cora_content:
      ...
      z = float(entries[3])
      a = float(entries[4])
      ax1s = float(entries[5])
      ax2s = float(entries[6])
      g = float(entries[7])
      v = float(entries[8])

      beam_features = map(float, [x,y,z,a,ax1s,ax2s,g,v])
      # words = map(float, entries[1:-1])
      example_id = entries[0]

      features = {
          'id': _bytes_feature(example_id),
          "beam_features": _float_feature(*beam_features),
          # 'x': _float_feature(x),
          # 'y': _float_feature(y),
          # 'z': _float_feature(z),
          # 'a': _float_feature(a),
          # 'ax1s': _float_feature(ax1s),
          # 'ax2s': _float_feature(ax2s),
          # 'v': _float_feature(v),
          # 'g': _float_feature(g),
